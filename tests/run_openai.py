...
if __name__ == "__main__":
    ...
    for effort in ["LOW", "MEDIUM", "HIGH"]:
        ...
        acc = []
        for sample in data:
            if sample["outputs"] is None:
                acc.append(0)
                continue
            output = json.loads(sample["outputs"][-1])
            acc.append(auto_verify("math", 1, [output["content"][0]["text"]], [sample["answer"]])[0])
        print(effort, np.mean(acc))
