...
# LOGDIR=/mnt/public/fuyu/mem/logs_workflow
# mkdir -p "$LOGDIR"
# exec > >(tee -i "$LOGDIR/$EXP.log") 2>&1
...
ray job submit --address="http://localhost:8265" \
    ...
    -- python -m marti.cli.commands.train --config-name "ma_exp_with_search" \
    async_workflow=True \
    ...
    default_agent.n_samples_per_prompt=1 \
    ...
    tools_config.tools.search.base_url="http://101.6.64.188:18080/retrieve" \
    tools_config.max_turns=2 \
    tools_config.num_workers=256 \
    tools_config.tools.search.rate_limit=128 \
    workflow_args.num_rounds=4 \
    workflow_args.memory_reward_style="propagate" \
    workflow_func_path="marti/worlds/workflows/experience_with_tool_workflow1.py" \
    ...
    wandb_run_name="${EXP}" \
    extra_eval_tasks=["2wiki","musique","bamboogle","hotpotqa"] \
    extra_eval_dir="${ROOT_DIR}/local/search-r1-bench-lite" \
