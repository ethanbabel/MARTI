...
export MARTI_LOGGING_LEVEL="WARN"
...
MODEL_DIR="/fs-computility/mabasic/qibiqing/marti-project/MARTI-Dev-Git/outputs/openrlhf/Qwen2.5-3B-Instruct-arpo-54k/"
SHORT_NAME="Qwen2.5-3B-Instruct-arpo-54k"
...
ENV_JSON=$(cat <<EOF
{
  ...
  "pip": ["hydra-core", "antlr4-python3-runtime==4.9.3", "shortuuid", "class_registry", "json5", "mcp[cli]", "func_timeout"]
}
EOF
)
...
ray job submit --address="http://localhost:8265" \
    ...
    -- python -m marti.cli.commands.train --config-name "tool_agent" \
    agent_workflow="tool" \
    ...
    default_agent.vllm_gpu_memory_utilization=0.75 \
    ...
    default_agent.save_steps=200 \
    default_agent.eval_steps=100 \
    ...
    default_agent.lr_warmup_steps=0 \
    ...
    tools_config.max_turns=4 \
    tools_config.num_workers=128 \
    tools_config.tools.search.base_url="http://101.6.64.188:18000/retrieve" \
    tools_config.tools.python.base_url="http://101.6.64.188:10086/run_code" \
    ...
    extra_eval_tasks=["2wiki","SimpleQA","aime24","aime25","bamboogle","hotpotqa","math500","musique"] \
    extra_eval_dir="${ROOT_DIR}/local/agent-bench" \
    ...
    wandb_run_name="${EXP}" \
    use_tensorboard="${TENSORBOARD}" 2>&1 | tee "${ROOT_DIR}/logs/std/${EXP}.log"
