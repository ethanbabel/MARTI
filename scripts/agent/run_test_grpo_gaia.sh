...
# SHORT_NAME="Qwen3-8B-step40"
# SHORT_NAME="qwen3-8b-sft_multiturn_full"
# SHORT_NAME="DeepSeek-R1-0528-Qwen3-8B"

for SHORT_NAME in "qwen3-8b-sft_multiturn_maj_full_merge"
do
TASK="gaia-mem-merge"
ALGO="gaia-val-sft-test-32k-t1.0"
...
PROMPT_DATA="json@${ROOT_DIR}/local/gaia-mem-allturns/${TASK}"
...
ray job submit --address="http://localhost:8265" \
    ...
    -- python -m marti.cli.commands.train --config-name "tool_mcp" \
    agent_workflow="tool" \
    ...
    default_agent.n_samples_per_prompt=32 \
    ...
    tools_config.max_turns=30 \
    mask_truncated_completions=True \
    eval_before_training=False \
    eval_only=True \
    eval_workers=4 \
    verify_task="gaia" \
    verify_task_eval="gaia" \
